...
export function authorPlain(
  ...
  return { id: a.id ?? '', handle: a.handle ?? '', displayName: a.displayName ?? '' };
}

/** Post as plain object so response is always JSON-serializable. */
export function postToPlain(p: Post | null | undefined): Record<string, unknown> | null {
  if (!p || typeof p !== 'object') return null;
  return {
    id: p.id ?? '',
    authorId: p.authorId ?? '',
    author: authorPlain((p as Post).author ?? null),
    ...
    lang: p.lang ?? null,
    createdAt: p.createdAt != null ? new Date(p.createdAt).toISOString() : undefined,
    updatedAt: p.updatedAt != null ? new Date(p.updatedAt).toISOString() : undefined,
...
export function replyToPlain(r: Reply | null | undefined): Record<string, unknown> | null {
  if (!r || typeof r !== 'object') return null;
  return {
    id: r.id ?? '',
    postId: r.postId ?? '',
    authorId: r.authorId ?? '',
    author: authorPlain((r as Reply).author ?? null),
    parentReplyId: r.parentReplyId ?? null,
    body: r.body ?? '',
    createdAt: r.createdAt != null ? new Date(r.createdAt).toISOString() : undefined,
    post: postToPlain((r as Reply).post ?? null),
  };
}
