...
import { filterVisiblePosts } from '../common/search-visibility.helper';
...
export class SearchController {
  ...
  async searchPosts(
    ...
    // Shared visibility check: protected profiles → only followers (and self)
    const visiblePostIds = await filterVisiblePosts(
      this.dataSource,
      this.followRepo,
      posts,
      _user?.id ?? null,
  ...
  async searchUsers(
    ...
    const getImageUrl = (key: string) => this.uploadService.getImageUrl(key);
    const excludeUserId = _user?.id ?? null;
    const hydrated = hits
      .filter((h) => {
        if (h.id != null && excludeUserId != null && h.id === excludeUserId)
          return false;
  ...
  async searchAll(
    ...
    const result = await this.meilisearch.searchAll(query, limit, topicId, offset);
    const getImageUrl = (key: string) => this.uploadService.getImageUrl(key);

    // Hydrate users (avatarUrl, filter pending, exclude self from results)
    const excludeUserId = _user?.id ?? null;
    ...
    // Hydrate posts (visibility, headerImageUrl) – uses shared visibility helper
