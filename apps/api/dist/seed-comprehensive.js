...
async function bootstrap() {
    ...
    const postTopicRepo = dataSource.getRepository(post_topic_entity_1.PostTopic);
    const followRepo = dataSource.getRepository(follow_entity_1.Follow);
    const replyRepo = dataSource.getRepository(reply_entity_1.Reply);
    const likeRepo = dataSource.getRepository(like_entity_1.Like);
    const keepRepo = dataSource.getRepository(keep_entity_1.Keep);
    const postEdgeRepo = dataSource.getRepository(post_edge_entity_1.PostEdge);
    const collectionRepo = dataSource.getRepository(collection_entity_1.Collection);
    const collectionItemRepo = dataSource.getRepository(collection_item_entity_1.CollectionItem);
    const mentionRepo = dataSource.getRepository(mention_entity_1.Mention);
    const externalSourceRepo = dataSource.getRepository(external_source_entity_1.ExternalSource);
    const topicFollowRepo = dataSource.getRepository(topic_follow_entity_1.TopicFollow);
    const inviteRepo = dataSource.getRepository(invite_entity_1.Invite);
    const settingsRepo = dataSource.getRepository(system_setting_entity_1.SystemSetting);
    ...
    `);
        ...
        for (let i = 0; i < numPosts; i++) {
            ...
            const post = postRepo.create({
                ...
                body: body,
                title: title,
                visibility: Math.random() < 0.9 ? post_entity_1.PostVisibility.PUBLIC : post_entity_1.PostVisibility.FOLLOWERS,
                lang: 'en',
                langConfidence: 0.95,
