    ...
    <div className="min-h-screen bg-ink">
      ...
      <header className="sticky top-0 z-50 bg-ink/95 backdrop-blur-md border-b border-divider px-4 py-3">
        <div className="max-w-[680px] mx-auto flex items-center justify-between">
          <Link href={`/post/${post.id}`} className="text-secondary hover:text-paper">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
          </Link>
          {post.title && (
            <h1 className="text-lg font-semibold text-paper truncate max-w-[60%]">{post.title}</h1>
          )}
      ...
      <article className="max-w-[680px] mx-auto px-6 py-12">
        ...
        <div className="flex items-center gap-3 mb-8 pb-6 border-b border-divider">
          ...
          <div>
            ...
            <div className="text-tertiary text-sm">{formatDate(post.createdAt)}</div>
          </div>
        </div>
        ...
        <div className="mt-16 space-y-12 pt-12 border-t border-divider">
          ...
          {/* Referenced By */}
          <section>
            <h2 className="text-2xl font-semibold mb-6 text-paper">Referenced by</h2>
            <div className="text-secondary text-sm">
              {post.quoteCount === 0 ? (
          ...
          {/* Quoted By */}
          <section>
            <h2 className="text-2xl font-semibold mb-6 text-paper">Quoted by</h2>
            <div className="text-secondary text-sm">
              {post.quoteCount === 0 ? (
                <p>Not quoted yet.</p>
              ) : (
...
function renderMarkdownForReading(text: string): string {
  ...
  let processed = text.replace(/^#\s+.+$/m, '');
  
  // Use shared renderer but with reading mode classes
  let html = renderMarkdown(processed);
  
  // Override classes for reading mode
  html = html.replace(/class="text-lg font-semibold/g, 'class="text-2xl font-semibold');
  html = html.replace(/class="text-xl font-semibold/g, 'class="text-3xl font-semibold');
  html = html.replace(/class="text-2xl font-bold/g, 'class="text-4xl font-bold');
  
  // Add reading mode link styling
  html = html.replace(/class="text-primary hover:underline font-medium"/g, 'class="text-primary hover:underline font-medium border-b border-primary/30 pb-0.5"');
