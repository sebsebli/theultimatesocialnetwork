...
export function MultiTargetSheet({ targets, onClose }: MultiTargetSheetProps) {
  const handleTargetClick = (target: Target) => {
    if (target.type === 'post' && target.id) {
      window.location.href = `/post/${target.id}`;
    } else if (target.type === 'topic' && target.slug) {
      window.location.href = `/topic/${target.slug}`;
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-paper truncate">{target.title}</div>
                  {target.excerpt && (
                    <div className="text-sm text-secondary line-clamp-2 mt-1">{target.excerpt}</div>
                  )}
                  <div className="text-xs text-tertiary mt-1">
                    {target.type === 'topic' && 'Topic'}
                    {target.type === 'post' && 'Post'}
                    {target.type === 'url' && new URL(target.url || '').hostname}
                  </div>
                </div>
              </div>
            </button>
