    ...
    <div className="border-b border-divider bg-ink hover:bg-white/[0.02] transition-colors group">
      <div className="px-5 pt-4 flex items-center gap-2">
        ...
        <p className="text-[13px] text-tertiary font-medium">
          Saved by{' '}
          <Link href={`/user/${post.author.handle || userName}`} className="text-paper font-semibold hover:text-primary transition-colors">
            {userName}
          </Link>
          {' '}to{' '}
          <Link href={`/collections/${collectionId}`} className="text-paper font-semibold hover:text-primary transition-colors">
            {collectionName}
          </Link>
        </p>
      </div>
