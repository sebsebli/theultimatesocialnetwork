        ...
        <div className="flex items-center gap-6 md:gap-8 text-sm font-medium">
          ...
          <div className="relative">
            <button 
              ...
              className="flex items-center gap-1 text-[#A8A8AA] hover:text-[#F2F2F2] transition-colors uppercase"
            >
              {lang}
            ...
            {isLangMenuOpen && (
              <div className="absolute top-full right-0 mt-2 bg-[#1A1A1D] border border-[#333] rounded-lg shadow-xl py-2 min-w-[120px] z-50 max-h-[60vh] overflow-y-auto custom-scrollbar">
                {(Object.keys(translations) as Language[]).map((l) => (
                  <button
                    key={l}
                    onClick={() => { setLang(l); setIsLangMenuOpen(false); }}
                    className={`block w-full text-left px-4 py-2 hover:bg-[#333] transition-colors uppercase ${lang === l ? 'text-[#F2F2F2]' : 'text-[#A8A8AA]'}`}
                  >
                    {l}
                  </button>
                ))}
