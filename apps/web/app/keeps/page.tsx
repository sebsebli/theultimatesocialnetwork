...
export default function KeepsPage() {
  ...
  return (
    <div className="max-w-[680px] mx-auto min-h-screen border-x border-divider bg-ink relative">
      {/* Header */}
                ...
                <Link href={`/post/${keep.post.id}`} className="block">
                  <div className="flex items-start gap-3 mb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-paper">{keep.post.author.displayName}</div>
                      <div className="text-xs text-tertiary">@{keep.post.author.handle} • {formatTime(keep.createdAt)}</div>
                    </div>
                  </div>
                  {keep.post.title && (
                    <h3 className="text-base font-bold text-paper mb-1">{keep.post.title}</h3>
                  )}
                  <p className="text-sm text-secondary line-clamp-2">{keep.post.body}</p>
                </Link>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-ink border border-white/10 rounded-xl w-full max-w-sm overflow-hidden shadow-xl">
            <div className="p-4 border-b border-white/10 flex justify-between items-center">
              <h3 className="text-lg font-bold text-paper">Add to Collection</h3>
              <button 
            ...
            <div className="max-h-[300px] overflow-y-auto p-2">
              {collections.length === 0 ? (
                <div className="p-4 text-center text-secondary text-sm">No collections found.</div>
              ) : (
                collections.map(col => (
                  <button
                    key={col.id}
