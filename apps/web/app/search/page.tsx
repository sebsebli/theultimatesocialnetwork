...
function SearchContent() {
  ...
  const [activeTab, setActiveTab] = useState<'people' | 'topics' | 'posts'>('people');
  ...
  const handleSearch = async (searchQuery: string) => {
    ...
    try {
      if (activeTab === 'posts') {
        const res = await fetch(`/api/search?q=${encodeURIComponent(searchQuery)}&type=posts`);
        if (res.ok) {
        ...
        const res = await fetch(`/api/search/topics?q=${encodeURIComponent(searchQuery)}`);
        if (res.ok) {
          const data = await res.json();
          setResults(data || []);
        }
      ...
      <div className="px-6 py-6">
        {loading ? (
          ...
          <div className="space-y-4">
            {activeTab === 'people' && results.map((user: any) => (
              <Link
                key={user.id}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-tertiary text-sm">@{user.handle}</div>
                    {user.bio && <div className="text-secondary text-sm mt-1">{user.bio}</div>}
                  </div>
            ...
            {activeTab === 'topics' && results.map((topic: any) => (
              <Link
                key={topic.id}
                href={`/topic/${topic.slug}`}
                className="block p-4 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 transition-colors"
              >
                <div className="font-semibold text-paper mb-1">{topic.title}</div>
                <div className="text-tertiary text-sm">{topic.postCount} posts</div>
              </Link>
            ))}
            ...
            {activeTab === 'posts' && results.map((post: any) => (
              ...
              >
                {post.title && (
                  <h3 className="font-semibold text-paper mb-2">{post.title}</h3>
                )}
                <p className="text-secondary text-sm line-clamp-2">{post.body}</p>
                <div className="text-tertiary text-xs mt-2">@{post.author.handle}</div>
              </Link>
            ))}
          </div>
        )}
...
export default function SearchPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-ink flex items-center justify-center text-secondary">Loading search...</div>}>
      <SearchContent />
    </Suspense>
  );
}