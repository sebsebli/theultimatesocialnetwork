...
export default function MessagesScreen() {
  ...
  const [threadMenuThread, setThreadMenuThread] = useState<ThreadItem | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        const data = await api.get<ChatSearchHit[]>(
          `/messages/search?q=${encodeURIComponent(q)}&limit=30`,
        );
  ...
  const handleMarkUnread = async (item: ThreadItem) => {
    ...
    try {
      await api.patch(`/messages/threads/${item.id}/read`, { read: false });
      fetchThreads();
    } catch (e) {
  ...
  const handleDeleteThread = async (item: ThreadItem) => {
    setThreadMenuThread(null);
    setDeleteConfirmThread(null);
    try {
      await api.delete(`/messages/threads/${item.id}`);
  ...
  return (
    <View style={styles.container}>
      <ScreenHeader
        ...
        right={
          <HeaderIconButton
            ...
            icon="add"
            accessibilityLabel={t("messages.new", "New message")}
          />
        }
      />
        ...
        searchLoading ? (
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <CenteredEmptyState
                icon="search-off"
                headline={t("search.noResults", "No results")}
                compact
              />
            }
            {...LIST_SCROLL_DEFAULTS}
      ...
      <OptionsActionSheet
        visible={!!threadMenuThread}
        title={
          threadMenuThread
            ? ((
                threadMenuThread.participant ||
                threadMenuThread.participants?.[0]
            ...
            ? [
                {
                  ...
                  onPress: () => handleMarkUnread(threadMenuThread),
                  icon: "mark-email-unread" as const,
                },
              ]
            : []),
          {
            ...
            onPress: () => {
              setThreadMenuThread(null);
              setDeleteConfirmThread(threadMenuThread!);
            },
            destructive: true,
            icon: "delete",
          },
        ...
        onCancel={() => setThreadMenuThread(null)}
      />

      <ConfirmModal
        visible={!!deleteConfirmThread}
        ...
        cancelLabel={t("common.cancel")}
        onConfirm={async () =>
          deleteConfirmThread && handleDeleteThread(deleteConfirmThread)
        }
        onCancel={() => setDeleteConfirmThread(null)}
        destructive
      />
    </View>
  );
