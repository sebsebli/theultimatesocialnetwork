...
export default function PostDetailScreen() {
  ...
  const handleDeletePost = async () => {
    try {
      ...
      showError(
        error?.message || t("post.deleteFailed", "Failed to delete post"),
      );
      throw error;
    }
  };
                    ...
                    onPress={() => {
                      if (source.type === "external" && source.url) {
                        Linking.openURL(source.url).catch((err: any) => {
                          ...
                          showError(
                            t("post.failedOpenUrl", "Failed to open URL"),
                          );
                        });
        ...
        {referencedBy.length > 0 && (
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <Text style={styles.sectionTitle}>
                {t("post.referencedBy", "Referenced by")}
              </Text>
              <Text style={styles.sectionCount}>
                {referencedBy.length} {t("post.replies")}
              </Text>
            </View>
            <View style={styles.sourcesList}>
              {referencedBy.slice(0, 2).map((refPost) => {
                ...
                return (
                  <SourceOrPostCard
                    ...
                    onPress={() =>
                      refPost.title
                        ? router.push(`/post/${refPost.id}/reading`)
                        : router.push(`/post/${refPost.id}`)
                    }
                  />
                );
              })}
