...
export default function OfflineStorageScreen() {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <ScreenHeader title={t('settings.manageOfflineStorage', 'Manage offline storage')} paddingTop={0} />

      <View style={styles.summary}>
        <Text style={styles.summaryText}>
          {t('settings.offlineStorageSummary', '{{count}} articles · {{size}}', {
            count: info.count,
        ...
        {info.count > 0 && (
          <Pressable style={styles.removeAllBtn} onPress={handleRemoveAll}>
            <Text style={styles.removeAllText}>{t('settings.removeAllOffline', 'Remove all')}</Text>
          </Pressable>
        )}
      </View>
            ...
            <View style={styles.row}>
              <View style={styles.rowContent}>
                ...
                {item.author?.displayName && (
                  <Text style={styles.rowAuthor}>{item.author.displayName}</Text>
                )}
              </View>
              <Pressable
      ...
      <ConfirmModal
        ...
        message={removeOnePost ? t('settings.removeDownloadConfirm', 'Remove "{{title}}" from this device?', {
          title: (removeOnePost.title || removeOnePost.body?.slice(0, 30) || removeOnePost.id).slice(0, 50),
        }) : ''}
      ...
      <ConfirmModal
        ...
        message={t('settings.removeAllOfflineConfirm', 'This will free storage. You can download articles again when online.')}
