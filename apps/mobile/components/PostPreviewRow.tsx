...
interface PostPreviewRowProps {
  post: {
    ...
    author?: { displayName?: string; handle?: string; avatarKey?: string | null; avatarUrl?: string | null } | null;
...
function PostPreviewRowInner({ post, fullWidth = false, isPrivateForViewer }: PostPreviewRowProps) {
  ...
  const displayTitle = showPrivateOverlay ? t('common.private', 'Private') : (post.title || bodyPreview || 'Post');
  ...
  return (
    ...
    >
      <View style={styles.thumbWrap}>
        ...
        {showPrivateOverlay && (
          <View style={[styles.privateOverlay, { width: THUMB_WIDTH, height: thumbHeight }]}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.authorRow}>
          <Avatar size={24} uri={authorAvatarUri} name={authorName || undefined} style={styles.authorAvatar} />
          {authorName ? <Text style={styles.author} numberOfLines={1}>{authorName}</Text> : null}
        </View>
        {!showPrivateOverlay && bodyPreview && !post.title ? <Text style={styles.bodyLine} numberOfLines={1}>{bodyPreview}</Text> : null}
      </View>
...
const styles = createStyles({
  ...
  authorRow: { flexDirection: 'row', alignItems: 'center', gap: SPACING.xs, marginBottom: 2 },
  authorAvatar: { marginRight: 0 },
  author: { fontSize: 13, color: COLORS.tertiary, fontFamily: FONTS.regular, flex: 1 },
